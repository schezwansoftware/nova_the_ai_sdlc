...
def build_server(config: OneDriveConnectorConfig, *, client: Optional[OneDriveClient] = None) -> FastMCP:
    ...
    server = FastMCP(
        ...
        instructions=(
            ...
            f"OneDrive sync folder allowlist: {config.allowed_directories}. "
            f"Readable file types are gated by this connector's configured "
            f"file_categories ({config.file_categories}) -- 'text' "
            "(.md/.markdown/.txt/.rst) is the default; 'code' (source/config "
            "files), 'office' (.docx/.xlsx/.pptx), and 'pdf' are opt-in via "
            "config, not auto-enabled. OCR/image-based text recognition is out "
            "of scope regardless of configured categories -- a deliberate "
            "exclusion, not a gap. Reads the OneDrive desktop client's "
            "already-synced local folder directly -- no Microsoft Graph API "
            "call, no OAuth. Every search/fetch resolves the real, "
            "symlink-followed path of each file and verifies it is actually "
