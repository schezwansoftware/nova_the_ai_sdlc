...
def fake_keyring(monkeypatch):
    ...
    import ai_sdlc.mcp_connectors.common as common_module
...
def test_connectors_config_dir_default_is_distinct_from_cli_config_dir(monkeypatch):
    monkeypatch.delenv("AI_SDLC_MCP_CONFIG_DIR", raising=False)
    path = connectors_config_dir()
    assert path.name == "ai-sdlc-mcp"
    assert "ai-sdlc-cli" not in str(path)
