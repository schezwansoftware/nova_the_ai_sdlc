...
from ai_sdlc.mcp_connectors.common import ConnectorAPIError, ConnectorAuthError, CredentialRef
from ai_sdlc.mcp_connectors.sharepoint.config import SharePointServerSiteConfig
from ai_sdlc.mcp_connectors.sharepoint.onprem_client import (
    ONPREM_SEARCH_PATH,
...
def _server_site(monkeypatch, secret="password", auth_method="basic") -> SharePointServerSiteConfig:
    ...
    return SharePointServerSiteConfig(
        ...
        username="jdoe" if auth_method == "basic" else "CONTOSO\\svc-ai-sdlc",
...
def test_ntlm_auth_uses_httpntlmauth_with_username_and_secret(monkeypatch):
    ...
    assert client._session.auth.username == "CONTOSO\\svc-ai-sdlc"
...
def test_ntlm_import_guard_raises_clear_connector_auth_error(monkeypatch):
    import ai_sdlc.mcp_connectors.sharepoint.onprem_client as onprem_module
