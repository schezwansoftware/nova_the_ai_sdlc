...
`ArchitectureAgent()` -> `retrieval_factory.get_default_retrieval_provider()`
raising `ProviderError` because `AI_SDLC_AGENT_FRAMEWORK=claude` is set but
`claude-agent-sdk` isn't installed), `AgentRegistry` used to swallow the
real exception unconditionally and report a misleading, contextless
"Agent not found: architecture" -- indistinguishable from the metadata
file genuinely not existing at all.

No network access / external credentials required -- this deliberately
exercises the *unconfigured-SDK* failure path, which doesn't need one.
"""
...
def test_construction_failure_is_recorded_not_silently_dropped(tmp_path, monkeypatch):
    ...
    # The agent genuinely failed to construct -- claude-agent-sdk isn't
    # installed in the test environment -- so `get()` still correctly
    # returns None...
    ...
    assert "claude-agent-sdk" in load_error or "ProviderError" in load_error
...
def test_orchestrator_surfaces_the_real_reason_instead_of_agent_not_found(tmp_path, monkeypatch):
    ...
    assert "claude-agent-sdk" in message or "ProviderError" in message
