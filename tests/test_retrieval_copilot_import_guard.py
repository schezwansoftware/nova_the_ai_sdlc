"""Verifies `retrieval_copilot.py` degrades gracefully when the optional
`github-copilot-sdk` extra isn't installed -- mirrors
`test_coding_copilot_import_guard.py` exactly, adapted to
`CopilotRetrievalProvider`/`RetrievalCapability`'s failure contract. The
module itself must always import cleanly, and only fail with a clear,
actionable `ProviderError` at `CopilotRetrievalProvider()` construction
time.
...
def test_module_imports_regardless_of_sdk_availability():
    module = importlib.import_module("ai_sdlc.capabilities.providers.retrieval_copilot")
    assert hasattr(module, "CopilotRetrievalProvider")
...
def test_constructor_raises_provider_error_when_sdk_unavailable(monkeypatch):
    ...
    with pytest.raises(ProviderError, match="github-copilot-sdk is not usable"):
        module.CopilotRetrievalProvider()
