...
Per the approved design, each server needs its own error contract that
does **not** depend on `ai_sdlc.capabilities`' `ProviderError`/
`MalformedResponseError` (those are internal to Nova's capability-layer
providers; a standalone MCP server must not require the rest of this
codebase to be importable). `ConnectorError` and its three subclasses
below are that self-contained contract.
...
`keyring` is imported defensively (deferred-ImportError pattern, mirroring
`capabilities/providers/coding_copilot.py`'s handling of the optional
`copilot` extra): this module always imports cleanly even in an
...
`connectors_config_dir()` (default `~/.config/ai-sdlc-mcp/`, overridable
via `AI_SDLC_MCP_CONFIG_DIR` for tests/CI) -- e.g. `jira.json`,
`confluence.json`, `sharepoint.json`. This mirrors the *shape* of
`cli/config.py`'s `CLIConfig` (`config_dir()`/`config_path()`/
`load_config()`/`save_config()`, a strict Pydantic model) but is a
deliberately separate directory/namespace, per the approved design's
explicit instruction not to bolt connector config onto `CLIConfig` --
see `cli/config.py`'s own docstring ("Lives in the CLI's own config
directory... never in the target workspace's `.ai-sdlc/`") for why that
model is scoped elsewhere. One JSON file per connector (rather than one
shared multi-connector file) is what makes "run `ai-sdlc-mcp-jira`
...
class ConnectorError(Exception):
    ...
    raises. Deliberately not `ai_sdlc.capabilities`' `ProviderError`/
    `MalformedResponseError` -- see module docstring's "Error contract"
    section for why, and for why no explicit MCP-error translation code
    is needed anywhere: FastMCP already converts any exception raised
    inside a tool function into a real `isError` MCP tool response."""
...
def _require_keyring() -> Any:
    if _KEYRING_IMPORT_ERROR is not None:
        raise ConnectorAuthError(
            ...
            "`pip install ai-sdlc[jira]` etc.) before storing or resolving "
...
def connectors_config_dir() -> Path:
    ...
    Deliberately a distinct namespace from `cli/config.py`'s
    `~/.config/ai-sdlc-cli/` -- these are two unrelated config surfaces
    with two unrelated override env vars, on purpose."""
    override = os.environ.get("AI_SDLC_MCP_CONFIG_DIR")
    if override:
        return Path(override)
    return Path.home() / ".config" / "ai-sdlc-mcp"
