...
**Enterprise Connectors (Jira/Confluence/SharePoint/Local Docs/OneDrive) shipped as five standalone MCP servers** (`packages/mcp-connectors/`, Nexus-owned, callable independently of Nova) — this is no longer deferred at the connector level. What remains unbuilt is **Nova's own agents actually consuming them**: see "Phase 2 Consumption Design" immediately below for the full, locked design — a harnessed coding-agent tool still has no native awareness of these sources on its own, so that gap is closed by Sage's design, not by this section's codebase-grounding decision.
...
### Phase 2 Consumption Design: Sage as an Isolated Sub-Agent (Design Locked 2026-08-16, Not Yet Implemented)

This is a separate axis from the dual-index path above — it answers *how a specialist agent reaches Sage's connectors at all*, not how Sage searches once asked. Locked via brainstorm, zero code written; documented here so the design survives independently of any one session's memory.
...
Full design rationale and the brainstorm's rejected alternatives are preserved in the repo's own `todo.md` under "Sage — Phase 2 Knowledge Consumption Design."
