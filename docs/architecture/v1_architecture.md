...
`RetrievalCapability` (§9) stays a single request/response call from the calling agent's perspective, exactly like `ReasoningCapability`/`DesignCapability` — one query in, one token-budgeted context result out — even though, as of V1, its provider does its own internal multi-step exploration to produce that result. This is the same asymmetry `CodingCapability` already has to absorb (an internally iterative provider behind a single-call interface), reused rather than re-solved: V1's `RetrievalCapability` provider for codebase grounding is the *same* harnessed-agentic-tool pattern as `CodingCapability`'s provider, permissioned down to read/search only — no edit, write, or command-execution tools are ever granted, so it structurally cannot modify the target repository, matching Tier 2's read-only boundary (below) even though the underlying tool is capable of far more. See §9 for the full provider decision and its tradeoffs against Sage's originally-scoped custom index.
...
Consumed by Tier 2 and Tier 3 agents (§8) via `RetrievalCapability`, distinct from §9.1's directly-injected Standards Layer.
...
**Enterprise Connectors (Jira/Confluence/SharePoint/Local Docs/OneDrive) shipped as five standalone MCP servers** (`packages/mcp-connectors/`, Nexus-owned, callable independently of Nova) — this is no longer deferred at the connector level. **Nova's own agents (PO/Architecture/UX) actually consuming them is now built too**: see "Phase 2 Consumption Design" immediately below — a harnessed coding-agent tool still has no native awareness of these sources on its own, so that gap is closed by Sage's design, not by this section's codebase-grounding decision.

### Future/Scale Path: Dual-Index Architecture (Not V1)

If the harnessed-agent approach above proves too slow or too expensive once real codebases get large, **Sage**'s originally-scoped dual-index design remains the documented upgrade path — built as a swap-in second `RetrievalCapability` provider, not a rewrite of the capability interface itself:
...
### Phase 2 Consumption Design: Sage as an Isolated Sub-Agent (Design Locked 2026-08-16, Implemented 2026-08-17)

This is a separate axis from the dual-index path above — it answers *how a specialist agent reaches Sage's connectors at all*, not how Sage searches once asked. Locked via brainstorm, then built as designed (branch `agents/sage-phase2-context-wiring`) with no deviation from the shape below — both Claude and Copilot got real providers, and PO/Architecture/UX (not Developer, whose output doesn't come from a `ReasoningCapability`-validated schema) got `needs_context`. Full implementation account, verified-vs-assumed SDK details, and the deferred-item list live in `todo.md`'s "Sage — Phase 2 Knowledge Consumption" section; this section stays the architectural reference.

- **Isolated sub-session, not a pre-fetch aggregator, not shared tools.** A calling agent (PO/Architecture/UX/Developer) never gets direct MCP tool access on its own reasoning session — `ReasoningCapability.complete()` is structurally zero-tool by deliberate design (§8), an anti-prompt-injection guarantee this must not break. Instead, the agent asks Sage a plain-language question; Sage runs its own separate, isolated, bounded agentic session (the same "bounded session, one task, structured output" shape already used by the Reasoning capability's own providers) with the enabled MCP connectors attached *only inside that sub-session*. Only the final distilled, cited answer crosses back — never the raw tool-call transcript — so the calling agent's context window stays small.
- **Framework-agnostic tool wiring.** One shared, provider-agnostic resolver turns "this workspace's enabled connectors" into MCP server specs + tool names; each agent-framework provider (Claude, Copilot, any future framework per §8.1) does only a thin translation into its own SDK's native tool-config mechanism — never re-solving "which connectors, what config" per framework.
- **Per-project connector enablement**, a new `.ai-sdlc/connectors.json` (deliberately separate from `CLIConfig` and from `AI_SDLC_AGENT_FRAMEWORK` — different questions), declared at `ai-sdlc init` via a checklist. Stores only which connectors are on + how to launch each server; a connector's own fine-grained scoping (allowed projects/spaces) stays in that connector's existing config, never duplicated here. A connector enabled but unconfigured is skipped quietly at use time, not a hard error.
- **Orion mediates, via the existing clarification mechanism, generalized.** Reuses the shipped `needs_clarification`/`clarification_question` schema pattern, generalized to a second flag (`needs_context`/`context_query`) a worker's own structured output can set when it judges it's missing something — preserving the zero-tool guarantee (a schema field, not a live tool call) while keeping "ask for help" driven by per-task judgment, not a blanket rule. Orion checks local memory first, asks Sage only if memory is empty, then resumes the worker with the merged answer — same resume shape as clarification/approval, except this resolves automatically with no human involved. Orion itself stays a pure messenger: it never needs to know connectors exist, or which one is relevant — that routing knowledge lives entirely inside Sage.
- **Full, structured, visible logging.** Every step (request → memory check → Sage invocation → answer + source + duration → resume) is written as a structured `.ai-sdlc/audit/` record, full detail rather than a summary, and narrated live in the CLI the same way an in-progress reasoning call already is. This serves two audiences: the human watching a run, and **Sentinel**, whose future evaluation work depends on this being real, structured data rather than prose.
- **Local memory, owned by Sage, not a new agent** — folds naturally into Sage's existing "context engineering" charter. Written immediately whenever Sage successfully answers something; no periodic consolidation for V1, matching this document's "no indexing/sync infrastructure until proven necessary" posture elsewhere in this section. The check itself stays a cheap, plain lookup, never another agentic session — the point is keeping the common case near-free. Each entry keeps its source and save-date so it can later be judged as possibly stale, not permanent truth.
- **Five resolved edge cases:** (1) retrieved content's safety relies on every connector already being read-only by construction — deeper prompt-injection sanitization of the *text* itself remains Aegis's pre-existing, still-open gap (§10), not solved here; (2) Sage finding nothing anywhere is a normal result, not an error — the worker's own next reasoning pass decides whether to proceed or escalate to a real clarifying question; (3) a connector that's configured but fails live is already covered by the connectors' existing MCP error contract (a clean `isError: true`, never a crash) — Sage just shouldn't treat one failed source as fatal; (4) which connector to check is always Sage's own call, never hinted by the caller; (5) Sage inherits the workspace's single `AI_SDLC_AGENT_FRAMEWORK` choice rather than getting its own.

Full design rationale, the brainstorm's rejected alternatives, and the implementation account (files changed, tests, what's deliberately deferred) are preserved in the repo's own `todo.md` under "Sage — Phase 2 Knowledge Consumption."
...
### Decision 6: `RetrievalCapability`'s V1 Codebase Provider Reuses `CodingCapability`'s Harness, Read-Only, Instead of Building Sage's Custom Index First

- **Reason:** `CodingCapability`'s harness (Decision 4) already solves "give an agentic tool a scoped, permissioned way to work against a real repository" — the only thing V1's `RetrievalCapability` needs on top is denying every write/execute tool so the same class of tool can only look, never change anything. Building Sage's originally-scoped Tree-Sitter/embedding/hybrid-search index (§9) is real, standalone infrastructure — an indexer, a re-index-on-change story, search-quality tuning — for a capability whose whole job ("explore a codebase and explain what's relevant") is already something these harnessed tools are good at without any of that. Shipping the smaller, already-half-built thing first and only reaching for the custom index if it proves insufficient follows the same "harness before you build" logic as Decision 4, applied one layer up.
- **Alternatives Rejected:** Building Sage's dual-index RAG stack as the V1 provider (rejected for V1: real infrastructure investment — indexing pipeline, re-index-on-change, embedding/reranking quality tuning — committed before any evidence the simpler harnessed-agent approach is insufficient; remains the documented upgrade path in §9, not abandoned). Giving Architecture/Review/Documentation the full `CodingCapability` (write/execute) instead of a read-only `RetrievalCapability` variant (rejected: directly violates Decision 5's tier boundary — these agents' whole role is to judge/advise, and a write-capable "grounding" step would let them silently act on the repository outside the approval gate).
- **What would change this:** large target repositories where per-call agentic exploration is measurably too slow or too costly relative to a fast index lookup — that's the trigger for building the dual-index provider, not a fixed timeline (§20, new open question).
